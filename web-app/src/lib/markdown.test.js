...
describe("parseSegments", () => {
	it("returns single text segment for plain text", () => {
		expect(parseSegments("hello world")).toEqual([{ type: "text", content: "hello world" }]);
	});

	it("extracts a single mermaid block", () => {
		const input = "```mermaid\ngraph TD\n  A --> B\n```";
		expect(parseSegments(input)).toEqual([{ type: "mermaid", content: "graph TD\n  A --> B" }]);
	});
