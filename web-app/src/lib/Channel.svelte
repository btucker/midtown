<script>
  ...
  function calculateAutocompletePosition() {
    ...
    // On mobile PWAs, when the keyboard opens, both getBoundingClientRect() and
    // position: fixed elements are positioned relative to the visual viewport (the
    // visible area after the keyboard opens), so they should align correctly.
    //
