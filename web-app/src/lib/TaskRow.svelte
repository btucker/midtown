...
// Status rollup for row variant: in_progress if any child is in_progress,
// done only when parent + all children are done
...
const childDerivedReviewer = $derived(
	!reviewer && children.length > 0 ? (children.find((c) => /review/i.test(c.subject))?.owner ?? null) : null,
);
const effectiveReviewer = $derived(
	isCard ? (relatedPr?.reviewer ?? childDerivedReviewer) : (reviewer ?? childDerivedReviewer),
);
