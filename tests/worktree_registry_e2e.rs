...
fn test_worktree_registry_integration_end_to_end() {
    ...
    let registry_response = fixture.rpc_call("worktree.list", None);

    if let Some(response) = registry_response {
        if response["error"].is_null() {
            let worktrees = response["result"]["worktrees"]
                .as_array()
            ...
            assert!(
                found_worktree.is_some(),
                "Task worktree should be registered in the worktree registry. Worktree ID: {}",
                worktree_id
            );

            if let Some(wt) = found_worktree {
                // Verify the worktree is bound to the coworker
                let current_coworker = wt["current_coworker"].as_str();
                assert_eq!(
                    current_coworker,
                    Some(coworker_name.as_str()),
                ...
                let task_id_str = wt["task_id"].as_str();
                assert_eq!(
                    task_id_str,
                ...
                let branch_name = wt["branch_name"].as_str();
                assert_eq!(
                    branch_name,
            ...
            eprintln!(
                "Warning: worktree.list RPC returned error: {:?}",
                response["error"]
            );
        }
    } else {
        eprintln!(
            "Warning: worktree.list RPC failed (worktree registry RPC may not be implemented yet)"
        );
    }
