...
//!
//! Tests plugin directory discovery (repo-level resolution) and event
//! serialization for Python workflow scripts. Socket-based dispatch tests
//! are placeholders until the Rust-Python socket communication is wired up.
//!
//! Note: The full 4-level resolution order (including local/home paths) is
//! tested in `src/paths_tests.rs` unit tests which can override the base dir.
//! These integration tests cover the repo-relative paths (levels 1 and 3)
//! that don't depend on `~/.midtown`.

use std::fs;

use tempfile::TempDir;

// ── Plugin directory discovery (repo-level) ──────────────────────────────────
//
// Levels 1 and 3 of the 4-level resolution are purely project_root-relative,
// so they work in integration tests without overriding the base directory.

#[test]
fn discovery_returns_none_when_no_workflow_files_exist() {
    let tmp = TempDir::new().unwrap();
    let project_root = tmp.path().join("project");
    fs::create_dir_all(&project_root).unwrap();

    // Use a repo name that won't match any real local workflow files
    let result = midtown::paths::workflow_script_for_channel(
        "test-channel-nonexistent",
        &project_root,
        "test-repo-workflow-plugin-tests-nonexistent",
    );
    assert!(
        result.is_none(),
        "Should return None when no workflow scripts exist"
    );
}

#[test]
fn discovery_finds_channel_specific_repo_script() {
    let tmp = TempDir::new().unwrap();
    let project_root = tmp.path().join("project");

    // Level 1: <project>/.midtown/channels/<channel>/workflow.py
...
#[test]
fn discovery_finds_project_default_repo_script() {
    let tmp = TempDir::new().unwrap();
    let project_root = tmp.path().join("project");

    // Level 3: <project>/.midtown/workflow.py
    let script = project_root.join(".midtown").join("workflow.py");
    fs::create_dir_all(script.parent().unwrap()).unwrap();
    fs::write(&script, "# project default workflow").unwrap();
...
#[test]
fn discovery_channel_specific_takes_priority_over_project_default() {
    let tmp = TempDir::new().unwrap();
    let project_root = tmp.path().join("project");

    // Level 1: channel-specific repo
    let channel_script = project_root
        .join(".midtown")
        .join("channels")
        .join("proj-auth")
        .join("workflow.py");
    fs::create_dir_all(channel_script.parent().unwrap()).unwrap();
    fs::write(&channel_script, "# channel-specific").unwrap();

    // Level 3: project default repo
    let default_script = project_root.join(".midtown").join("workflow.py");
    fs::write(&default_script, "# project default").unwrap();
    ...
    assert_eq!(
        result,
        Some(channel_script),
        "Channel-specific script should win over project default"
    );
}

#[test]
fn discovery_different_channels_can_have_different_scripts() {
    let tmp = TempDir::new().unwrap();
    let project_root = tmp.path().join("project");

    let script_a = project_root
        .join(".midtown")
        .join("channels")
        .join("proj-auth")
        .join("workflow.py");
    fs::create_dir_all(script_a.parent().unwrap()).unwrap();
    fs::write(&script_a, "# auth workflow").unwrap();

    let script_b = project_root
        .join(".midtown")
        .join("channels")
        .join("proj-frontend")
        .join("workflow.py");
    fs::create_dir_all(script_b.parent().unwrap()).unwrap();
    fs::write(&script_b, "# frontend workflow").unwrap();

    let repo = "test-repo-workflow-plugin-tests-nonexistent";
    let result_a = midtown::paths::workflow_script_for_channel("proj-auth", &project_root, repo);
    let result_b =
        midtown::paths::workflow_script_for_channel("proj-frontend", &project_root, repo);

    assert_eq!(result_a, Some(script_a));
    assert_eq!(result_b, Some(script_b));
    assert_ne!(
        result_a, result_b,
        "Different channels should resolve to different scripts"
    );
}

#[test]
fn discovery_channel_without_script_falls_back_to_project_default() {
    let tmp = TempDir::new().unwrap();
    let project_root = tmp.path().join("project");
    let repo = "test-repo-workflow-plugin-tests-nonexistent";

    // Channel A: has its own script
    let script_a = project_root
        .join(".midtown")
        .join("channels")
        .join("proj-auth")
    ...
    // Project default: shared fallback
    let default_script = project_root.join(".midtown").join("workflow.py");
    fs::write(&default_script, "# default workflow").unwrap();
    ...
    assert_eq!(
        result_b,
        Some(default_script),
        "Channel without its own script should fall back to project default"
    );
}
