...
**`create_fork_session` helper:** A `pub(crate)` shared helper in `rpc_session.rs`, callable from `handle_session_fork` (explicit CLI call), `handle_session_fork_thread` (web UI fork), and the channel lead's own `session.fork` RPC. The `channel_hint` parameter supplies the known channel name even when session records are stale.
...
**Thread ownership broadcast:** When a fork is created or destroyed (including fork process death), the daemon broadcasts a `ThreadOwnership` `WebUpdate` to all WebSocket clients. This contains the `thread_parent_id`, `channel`, and `has_dedicated_session` flag. The frontend's `threadOwnership` store tracks ownership per thread.
...
- Push notifications (W3C Push API with VAPID)
  ...
  - **Triggers**: Only two event types fire push notifications: (1) `@user`/`@{display_name}` mentions in channel posts (`rpc_channel.rs`), and (2) task completions (`dispatch.rs` via `task_completed_effects()`). Mentions use `send_push_notification()` directly; task completions use the `Effect::SendPushNotification` pattern.
  - **Tags**: Each notification uses a unique tag (e.g., `task_completed_{id}`, `mention`) to prevent the Web Notifications API from silently replacing unread notifications.
