...
**Initial nudge on fork:** Both the CLI and web-UI fork paths send a `NudgeSession` to fresh forks so they have an initial message to act on (without a nudge, forks sit idle forever). The CLI path (`handle_session_fork`) accepts an optional `initial_message` parameter; when provided, it's used as the nudge text. Otherwise, channel-lead callers get `fork_initial_framing` (role-specific instructions). Non-channel-lead callers (e.g. the project lead) receive no automatic framing — the framing text assumes a channel-lead role which would be misleading for the main lead.
