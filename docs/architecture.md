...
- `tasks_with_running_sessions: HashSet<String>` — (top-level, internal to snapshot construction) Task IDs that have an actively running session. Computed from `session_task_map` by checking both `SessionRecord.is_running` and `active_session_ids` (the union of both signals). The `active_session_ids` check is necessary because `spawn_coworker` uses `or_insert_with` for existing session records, leaving `is_running=false` even after a successful resume — `active_session_ids` is the authoritative liveness signal from the process monitor. Used as input to `is_task_pr_protected()`.
- `pr_protected_tasks: HashSet<String>` — (top-level) Task IDs that should not be spawned or recovered due to PR status or task completion. Pre-computed from `all_tasks` during `collect_world_snapshot()` via `is_task_pr_protected()`. Checks: task completed, explicit `task.pr` in merged PR cache, task has an open PR (via `tasks_with_open_prs`) **and** a running session (via `tasks_with_running_sessions`), or GitHub PR title pattern match (via `github_open_pr_task_ids`) **and** a running session. Open-PR checks only block dispatch when there is evidence someone is actively working on the PR — without a running session, the task is dispatchable. This prevents a catch-22 where a pending task linked to an existing PR could never be assigned. Dispatch functions use `snap.pr_protected_tasks.contains()` instead of per-call-site guard functions.
