...
1. **Agent definition (Layer 1)** — Role identity and behavioral instructions, stored in `.claude/agents/midtown-*.md` (project-level) so Claude Code's `--agent` flag discovers them directly. The binary also embeds these as fallbacks, and `~/.claude/agents/` (user-level) is checked as well. For Claude Code sessions, Layer 1 is delivered via the `--agent <name>` CLI flag (e.g., `--agent midtown-code-author`), while Layers 2+3 go through `--append-system-prompt`. For Codex sessions (which don't support `--agent`), all layers are bundled into `--system-prompt`. The mapping from `CoworkerRole` to agent name is in `CoworkerRole::agent_name()`, and `render_append_prompt()` returns only Layers 2+3. The `--agent` flag is passed on both fresh and resume sessions to support task handoff via `--resume --agent`.
