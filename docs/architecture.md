...
- `pr_protected_tasks: HashSet<String>` — (top-level) Task IDs that should not be spawned or recovered due to PR status or task completion. Pre-computed from `all_tasks` during `collect_world_snapshot()` via `is_task_pr_protected()`. Checks (in order): (1) task completed → always protected; (2) owner not in `active_names` → never protected (allows dispatch of pending tasks or tasks whose owner went away, even if they have open PRs); (3) explicit `task.pr` in merged PR cache; (4) task has an open PR (via `tasks_with_open_prs`); (5) GitHub PR title pattern match (via `github_open_pr_task_ids`). Dispatch functions use `snap.pr_protected_tasks.contains()` instead of per-call-site guard functions.
