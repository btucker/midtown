...
This constraint applies to **all functions called from `evaluate_tick()`**, not just those in `rules.rs`. Decision-phase functions in domain modules (`pr.rs`, `dispatch.rs`, `health.rs`) take `&DaemonPersistentState` and `&[Task]` and return `Vec<Effect>` without performing I/O. `prepare_tick()` in `tick.rs` populates ephemeral `tick_*` fields on `DaemonPersistentState` once per tick from `DaemonState` caches (coworker lists, cooldowns, PR poll data, etc.). When adding or modifying decision logic, prefer the pure pattern: no `.await`, no `state.persistent_state.lock()`, no `session_manager.is_alive()`, no direct state queries. If data is needed for a decision, add it as a `tick_*` field on `DaemonPersistentState` and populate it in `prepare_tick()`.
...
The `blocks_map` (`HashMap<String, Vec<String>>`) is the inverse of `Task.blocked_by`: for each task X that appears in some task Y's `blocked_by`, map X → [Y, ...]. It is computed during snapshot collection and added to `DaemonPersistentState`. Only pending/in-progress tasks participate — completed tasks are excluded.
...
**Task assignment persistence**: `sessions[].task_id` on `SessionRecord` is the single source of truth for coworker→task mapping. `RecordTaskAssignment` (emitted by both `SpawnSession` and `NudgeCoworker` callback chains) updates two things: (1) `sessions[].task_id` in `DaemonPersistentState` so that insight routing, busy tracking, and other session-aware features can resolve the task's channel, and (2) the `task_to_session` reverse map for dispatch lookups. The `DaemonPersistentState::session_task_map` field (task_id → session_id) is built from session records during `prepare_tick()`, enabling O(1) lookup of "which session is working on task X?" via `DaemonPersistentState::find_session_for_task()`. The `DaemonPersistentState::name_task_assignments` field is also derived from session records, ensuring dispatch decisions always reflect the persisted state. The `DaemonPersistentState::busy_coworkers` field is derived from sessions cross-referenced with in-progress tasks (not from task file owners).
...
**DaemonPersistentState** carries both `dir_key` and `project_name` as separate fields. Decision functions use `ps.tick_dir_key` for path/config/task operations and `ps.tick_project_name` for identity checks. Old JSON fixtures with `repo_name` are supported via `#[serde(alias = "repo_name")]` on `dir_key`.
...
- **Health**: `ensure_lead_alive()` and `maybe_refresh_lead_session()` compare against `ps.tick_project_name` (`src/daemon/health.rs`)
...
- `fork_bound_threads` is rebuilt on startup from persisted `SessionRecord.bound_thread_id` entries, which keeps thread-bound coworkers routed correctly across restarts (including auto-binding spawned tasks via `task_thread_id`). Entries created directly by `create_fork_session` remain ephemeral. `spawn_coworker()` also populates `fork_bound_threads` for non-reviewer coworkers that inherit a `bound_thread_id` from their task — reviewers are excluded because they have their own respawn path (`decide_dead_reviewer_respawns`) and should not trigger fork crash recovery.

**Crash recovery:** When a fork process dies (API error, rate limit, etc.), the daemon detects it in the `session_drain_interval` handler and respawns the fork bound to the same thread. Detection happens **before** `cleanup_coworker_state` runs — the fork bindings (`fork_bound_threads`, `fork_bound_channels`) are captured first, then cleanup proceeds, then `Effect::RespawnFork` is executed. This ordering is critical: `cleanup_coworker_state` removes `topic_sessions` entries, so any detection path that reads `topic_sessions` after cleanup (e.g., snapshot-based) would never see dead forks. The `respawn_fork` function in `effects.rs` uses a **resume-first strategy**: when `old_session_id` is provided, it attempts to resume the fork's previous session (preserving conversation history) before falling back to a fresh spawn. The `old_session_id` is passed from both the `session_drain_interval` handler (proactive crash detection) and `try_lazy_fork_respawn` (lazy respawn on message delivery). On success, it re-establishes `topic_sessions` and reverse maps, sends a `NudgeSession` so the fork has an initial message (simplified for resumed sessions since they already have context), and broadcasts `ThreadOwnership(true)` to restore the web UI indicator. On failure, it removes the `topic_sessions` entry so the caller can detect the failure. Per-fork cooldown (`process_respawn` category, `ZOMBIE_RESPAWN_COOLDOWN`) prevents respawn loops. A per-thread max-retry limit (`MAX_FORK_RESPAWN_ATTEMPTS`, default 3) caps total respawn attempts tracked in `DaemonState::fork_respawn_counts`. When the limit is exhausted, the daemon cleans up the `topic_sessions` entry so thread replies fall back to the channel lead, posts a diagnostic message (including stderr from the last crash) to `#ops`, and removes the counter. The retry counter is only incremented when a respawn is actually attempted (i.e., cooldown has expired), not when the cooldown blocks the respawn.
...
- `channel.post` — Append a message to a channel; handles `/me` actions, @mention routing, review note deduplication, thread ID validation (rejects posts with a `thread_id` that doesn't match any existing message, preventing invisible "black hole" messages). Params: `sender`, `content`, `thread_id` (optional), `channel` (defaults to "midtown").
...
- **Polling path** (backstop): `detect_and_block_external_prs` in `pr.rs` compares `headRepositoryOwner.login` against `repo_owner` from `DaemonPersistentState`. Uses `"{owner}/fork"` as a placeholder repo name (the polling API doesn't expose the full fork repo name).
...
- **Stuck restart** (`health.rs`, `build_reviewer_respawn_effects`): Uses `DaemonPersistentState::channel_for_pr()` — the synchronous equivalent for decision functions operating on the snapshot. Resolves `escalation_target` from `DaemonPersistentState::channel_lead_names()`.
...
`DaemonPersistentState::channel_for_pr()` and `PrContext::get_channel()` are parallel implementations of the same two-step lookup for the sync (snapshot) and async (persistent state) contexts respectively.
...
**DaemonPersistentState fields** (reviewer fields are in the `reviewer: SnapshotReviewerState` sub-struct):
- `reviewer.reviewer_in_progress_comment_ids: HashMap<u64, u64>` — Maps PR number to the GitHub comment ID of a dangling "Review in progress" placeholder comment. Collected during `prepare_tick()` using `reviewer_placeholder_cache` (TTL: 120s for all entries, both positive and negative). Used by health functions to emit `UpdatePrComment` effects marking abandoned placeholders.
...
- `is_at_task_limit: bool` — (top-level) Pre-computed from `in_progress_tasks.len() >= max_in_progress_tasks` during `prepare_tick()`. Used by pure decision functions in `dispatch.rs` and `pr.rs` to gate new spawns without performing I/O. All task types (dev, reviewer, ops) share this single limit — there is no separate `REVIEW_HEADROOM` or dev-vs-reviewer distinction. Orphaned tasks (in-progress but no running coworker) count toward the limit while recovery is in progress. For RPC handlers outside the tick pipeline, `DaemonState::is_at_task_limit()` reads from disk directly.
- `pr_protected_tasks: HashSet<String>` — (top-level) Task IDs that should not be spawned or recovered due to PR status or task completion. Pre-computed from `all_tasks` during `prepare_tick()` via `is_task_pr_protected()`. Checks (in order): (1) task completed → always protected; (2) merged PR (via `pr_task_index.session_pr_for_task()` or `task.pr` in merged cache) → always protected regardless of session state (prevents recovery-loops); (3) owner not in `active_names` → not protected by open PRs (allows dispatch of pending tasks or tasks whose owner went away); (4) task has an open PR (via `pr_task_index.session_pr_for_task()`); (5) GitHub PR title pattern match (via `pr_task_index.github_pr_for_task()`). Dispatch functions use `ps.tick_pr_protected_tasks.contains()` instead of per-call-site guard functions.
