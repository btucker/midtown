...
**Cursor recovery after rotation:** The reset doesn't cause a false spike in unread messages because the next read operation rebuilds the cursor. When `read_since_cursor()` runs with `position: 0`, it re-reads all retained messages in `current.jsonl` and calls `cursor.update(new_position, last_message_id)`, restoring both fields. Similarly, callers like the chat TUI that use `set_cursor_to_end()` will get a `last_message_id` drawn from all history files (archives + current), so unread-count calculations remain accurate across the rotation boundary. The transient window between `reset()` and the next read is invisible to users because no unread-count calculation runs during rotation (the daemon holds the exclusive lock).
