...
**Daemon state garbage collection**: On every `PrPollTick`, `check_for_state_gc()` runs alongside stale worktree cleanup. It produces a single `GarbageCollectState` effect that atomically: (1) removes dead session records — reviewer sessions immediately, other stopped sessions after the retention period; (2) strips `initial_prompt` from stopped-but-retained sessions to reclaim space; (3) prunes orphaned task metadata map entries (`task_channel`, `task_model`, `task_plan`, `task_execution_skill`, `task_thread_id`, `task_message_id`) whose task IDs are not referenced by any session or active task. All three use the same `worktree_cleanup_retention_hours` config (default 24h). The GC is a pure decision function in `health.rs` — no I/O, returns `Vec<Effect>` — with the effect handler in `effects.rs` performing the mutations and posting a summary to the ops channel.
