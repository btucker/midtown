...
- `midtown coworker screenshot <url>` — runs Playwright locally to capture a screenshot, saves it to `~/.midtown/projects/<repo>/screenshots/{uuid}.png`, and prints an `[Attached: /path]` reference for channel posts. With `--github`, outputs `![alt](URL)` markdown for embedding in GitHub PR descriptions; the base URL is determined by `GlobalConfig`: if `webserver.external_url` is set, it is used as-is (e.g. `https://macbook-pro.taile2dd2b.ts.net:47022`); otherwise the scheme (http/https) is derived from TLS settings and combined with `localhost`. Screenshots are served via `GET /api/projects/<repo>/screenshots/<filename>` on the shared gateway (port 47022) and `GET /api/screenshots/<filename>` on the per-project daemon. No daemon RPC connection is needed for capture.
