...
- `pr_protected_tasks: HashSet<String>` — (top-level) Task IDs that should not be spawned or recovered due to PR status or task completion. Pre-computed from `all_tasks` during `collect_world_snapshot()` via `is_task_pr_protected()`. Checks: task completed, explicit `task.pr` in merged PR cache, task has an open PR (via `tasks_with_open_prs`), or GitHub PR title pattern match (via `github_open_pr_task_ids`). Dispatch functions use `snap.pr_protected_tasks.contains()` instead of per-call-site guard functions.
