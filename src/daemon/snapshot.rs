...
pub struct WorldSnapshot {
    ...
    /// Used by dispatch.rs to build the appropriate LaunchConfig when spawning.
    #[serde(default)]
    pub task_agent_type_map: HashMap<String, String>,
    /// Task-to-PR-number mapping for reviewer tasks.
    /// Maps task ID → PR number. Used by dispatch.rs to build reviewer LaunchConfig
    /// with the correct PR context.
    #[serde(default)]
    pub task_pr_number_map: HashMap<String, u64>,
...
pub(crate) async fn collect_world_snapshot(state: &DaemonState) -> WorldSnapshot {
    ...
    // task-to-pr-number, channel-lead, and lead-driven mappings
    let (
        task_channel,
        task_model_map,
        task_plan_map,
