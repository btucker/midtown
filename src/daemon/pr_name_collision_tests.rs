...
async fn test_reviewer_allocation_excludes_active_session_names() {
    ...
    let has_create_reviewer_task = effects
        .iter()
        .any(|e| matches!(e, Effect::CreateReviewerTask { pr_number, .. } if *pr_number == 7777));

    assert!(
        has_create_reviewer_task,
        "Expected a CreateReviewerTask effect for PR #7777. \
         Reviewer name selection (excluding active sessions) is now handled by the dispatch system."
    );
}
