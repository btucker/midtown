...
/// When the PR author is resolved from session data, they appear as excluded-author
/// in the "no reviewer available" diagnostic message.
#[test]
fn format_no_reviewer_reason_with_known_author() {
    // The resolved author should appear as excluded-author in the diagnostic
    let busy = vec!["madison".to_string(), "york".to_string()];
    let reason = format_no_reviewer_reason(&busy, Some("york"));
    assert_eq!(
        reason, "no eligible reviewers (busy: [madison, york], excluded-author: york)",
        "excluded-author should appear when session-based author is known"
    );
}
...
///
/// This test demonstrates that a PR with:
/// - A valid coworker branch name (york/fix-auth)
/// - A merge conflict (mergeable: "CONFLICTING")
/// - No active session owner
///
/// ...generates an orphan warning effect.
#[tokio::test]
...
async fn test_lead_pr_without_task_id_should_not_be_orphaned() {
    ...
    // Verify we got a CreateReviewTask effect for the reviewer
    let has_reviewer_spawn = effects
        .iter()
        .any(|e| matches!(e, Effect::CreateReviewTask { .. }));
    assert!(
        has_reviewer_spawn,
        "Expected CreateReviewTask effect for lead PR. Effects: {:#?}",
        effects
    );
}
...
/// A PR with a task-based branch name and a merge conflict but no active session
/// owner should generate an orphan warning.
///
/// This test verifies that a PR with:
/// - A task-based branch name ("task-42-fix-auth")
/// - A merge conflict (mergeable: "CONFLICTING")
/// - No session owner (coworker went on break)
///
/// ...generates an orphan warning effect.
#[tokio::test]
#[allow(clippy::await_holding_lock)] // Intentionally hold PATH_LOCK across await to prevent test interference
async fn test_orphaned_pr_with_task_branch_and_merge_conflict_is_ignored() {
    ...
    assert!(
        !effects.is_empty(),
        "Expected effects for orphaned PR with merge conflict. \
         The PR with branch 'task-42-fix-auth' and merge conflict should generate a warning."
    );
...
async fn test_reviewer_spawns_when_worktree_exists_but_no_current_coworker() {
    ...
    // empty active_names - no session owner (mirrors snapshot after restart)
...
async fn test_lead_pr_with_non_standard_branch_gets_reviewer() {
    ...
    assert!(
        has_create_review_task,
        "Expected CreateReviewTask effect for PR #1198. \
...
async fn test_reviewer_not_assigned_to_pr_author() {
    ...
    let has_create_review_task = effects
        .iter()
        .any(|e| matches!(e, Effect::CreateReviewTask { pr_number, .. } if *pr_number == 9998));

    assert!(
        has_create_review_task,
        "Expected a CreateReviewTask effect for PR #9998. \
         The PR needs review and reviewer name allocation is now handled by dispatch."
    );
}
...
async fn test_reviewer_spawn_aborted_on_worktree_collision_with_active_coworker() {
    ...
    // CreateReviewTask is emitted — worktree collision detection is now handled by
    // the dispatch system, not by collect_reviewer_effects_with_source.
    let has_review_task = effects
        .iter()
        .any(|e| matches!(e, Effect::CreateReviewTask { .. }));
    assert!(
        has_review_task,
        "CreateReviewTask should be emitted; worktree collision detection is now handled by dispatch."
    );
}
...
async fn test_reviewer_spawn_aborted_on_worktree_collision_mixed_case() {
    ...
    // CreateReviewTask is emitted — collision detection (including case normalization)
    // is now handled by the dispatch system, not by collect_reviewer_effects_with_source.
    let has_review_task = effects
        .iter()
        .any(|e| matches!(e, Effect::CreateReviewTask { .. }));
    assert!(
        has_review_task,
        "CreateReviewTask should be emitted; collision detection is now handled by dispatch."
    );
}
...
async fn test_reviewer_spawn_blocked_by_stale_active_names_retries_next_tick() {
    ...
    // With CreateReviewTask, collision detection is deferred to dispatch.
    // Both ticks should emit CreateReviewTask regardless of stale active_names.
    let has_spawn_tick1 = effects_tick1
        .iter()
        .any(|e| matches!(e, Effect::CreateReviewTask { .. }));
    assert!(
        has_spawn_tick1,
        "Tick 1: CreateReviewTask should be emitted; collision detection is now handled by dispatch."
    );

    // Tick 2: active_names refreshed, vernon no longer present → spawn also proceeds
    ...
    assert!(
        has_spawn_tick2,
        "Tick 2: CreateReviewTask should be emitted after active_names is refreshed."
    );
}
...
/// A coworker with no assignment should NOT block PrApproved — without PR-specific
/// evidence, suppressing would incorrectly gate unrelated PRs.
#[test]
...
fn augment_reviewer_catches_assignment() {
    ...
    assert!(
        ctx.has_active_reviewer,
        "Should flag active reviewer when assignment exists"
    );
}
...
fn augment_reviewer_ignores_assignment_for_different_pr() {
    ...
    assert!(
        !ctx.has_active_reviewer,
        "Should NOT flag active reviewer when assignment is for a different PR"
    );
}
...
async fn test_reviewer_spawn_inherits_task_channel() {
    ...
    // Extract the CreateReviewTask effect and check its channel
    let review_task = effects.iter().find_map(|e| {
    ...
    assert!(
        review_task.is_some(),
        "Expected a CreateReviewTask effect. Effects: {:#?}",
        effects
    );
    ...
    assert_eq!(
        ...
        "CreateReviewTask.channel should be set to the task's topic channel '{}'. \
...
async fn test_reviewer_spawn_no_channel_when_no_task_association() {
    ...
    {
        let mut ps = state.persistent_state.lock().await;
        // Use a task_id with no channel mapping — that's what makes channel: None in the effect.
        // The session identifies the PR as non-orphaned, but task_channel has no entry for
        // "task-77771", so the CreateReviewTask effect gets channel: None.
        ps.sessions.insert(
            "sess-madison-77771".to_string(),
            crate::daemon::state::SessionRecord {
    ...
    assert_eq!(
        review_task.unwrap(),
        None,
        "CreateReviewTask.channel should be None when the PR has no task association."
    );
}
...
async fn test_reviewer_spawn_includes_post_pr_comment_on_success() {
    ...
    // The function should emit a CreateReviewTask effect for this PR.
    // PostPrComment (placeholder) is now handled by the dispatch system.
    let has_create_review_task = effects
        .iter()
        .any(|e| matches!(e, Effect::CreateReviewTask { pr_number: pn, .. } if *pn == pr_number));
...
async fn test_placeholder_body_has_correct_tags_no_escaped_exclamation() {
    ...
    // The function should emit a CreateReviewTask effect for this PR.
    // PostPrComment (placeholder with correct tags) is now handled by the dispatch system.
    let has_create_review_task = effects
        .iter()
        .any(|e| matches!(e, Effect::CreateReviewTask { pr_number: pn, .. } if *pn == pr_number));
