...
///
/// Uses `session_by_thread()` to check for existing forks and `pending_forks` to
/// prevent duplicate concurrent spawns:
/// 1. Query SessionRecord for existing fork bound to this thread
/// 2. If found, verify the session is still alive
/// 3. If no live fork, reserve the slot in `pending_forks` and spawn
///
/// On success the `pending_forks` entry is removed (SessionRecord is the source of
/// truth); on failure it is also removed so the slot is available for retry.
///
...
/// true when a live fork was found via `session_by_thread()` (stale/dead entries are
/// ignored). The `fork_channel` is the resolved channel for the fork session (`None`
/// only for pre-existing forks).
...
pub(super) async fn create_fork_session(
    ...
    // Check if a concurrent fork spawn is already in progress.
    {
    ...
    // Query SessionRecord for an existing fork bound to this thread.
    let existing_record = {
    ...
    if let Some((ref existing_sid, ref session_name)) = existing_record {
        ...
        // relaunched. Without this check, new fork requests silently return
        // a dead session_id.
        let is_alive = if !session_name.is_empty() {
            state.session_manager.is_alive(session_name).await
        } else {
            false
        };
        if is_alive {
        ...
        // Proceed to create a fresh fork.
        warn!(
            "{}: existing fork for thread {} is dead (session_id={}, name={:?}) — spawning new fork",
            caller, thread_parent_id, existing_sid, session_name
        );
    }

    // Reserve the slot to prevent concurrent forks for the same thread.
    // Re-check after acquiring the lock to handle races.
    {
        let mut pending = state.pending_forks.lock().unwrap();
        if pending.contains(thread_parent_id) {
            return Err("fork in progress for this thread".to_string());
        }
        pending.insert(thread_parent_id.to_string());
    }

    // Re-check SessionRecord after reserving — a concurrent fork may have
    // completed between our initial check and the pending_forks reservation.
    {
        let ps = state.persistent_state.lock().await;
        if let Some(record) = ps.session_by_thread(thread_parent_id) {
            let concurrent_alive = if !record.name.is_empty() {
                state.session_manager.is_alive(&record.name).await
            } else {
                false
            };
            if concurrent_alive {
                // Release the pending slot — we're returning an existing session.
                state.pending_forks.lock().unwrap().remove(thread_parent_id);
                return Ok((record.session_id.clone(), true, None));
            }
        }
    }
        ...
        .await
    {
        Ok(sid) => sid,
        Err(e) => {
            // Release the pending slot — spawn failed, so the slot is available again.
            state.pending_forks.lock().unwrap().remove(thread_parent_id);
    ...
    // SessionRecord is persisted above — thread binding and channel routing
    // are derived from SessionRecord.bound_thread_id and SessionRecord.channel.

    // Release the pending_forks slot — SessionRecord is now the source of truth.
    state.pending_forks.lock().unwrap().remove(thread_parent_id);
...
///
/// Looks up the fork session via `session_by_thread()`, triggers `ShutdownSession`
...
pub(super) async fn handle_session_unfork_thread(
    ...
    // which marks the SessionRecord as non-running.
...
pub(super) async fn handle_session_thread_ownership(
    ...
    let (fork_session_id, owner, parent_lead) = {
        let ps = state.persistent_state.lock().await;
        let record = ps.session_by_thread(thread_parent_id);
        let fork_sid = record.map(|s| s.session_id.clone());
        let owner = record.map(|s| s.name.clone());
        let parent_lead = if fork_sid.is_some() {
            ps.channel_lead_sessions
                .get(channel)
