//! Kanban board data handlers and helpers.
//!
//! Contains the `kanban.data` RPC handler and supporting functions for
//! fetching PR data via GraphQL, computing CI status, and extracting
//! author/reviewer info from PR metadata.
...
pub(super) async fn handle_kanban_data(id: RequestId, state: &DaemonState) -> Response {
    ...
    let coworkers_data = collect_coworker_kanban_data(state, &prs, &reviewer_pr_map).await;
...
// ============================================================================
// Kanban data collection helpers
// ============================================================================

/// Collect coworker status data for the kanban board.
///
/// Maps active coworkers to their workflow phase, task ID, PR number,
/// and health status for display on the board.
async fn collect_coworker_kanban_data(
    state: &DaemonState,
    prs: &[serde_json::Value],
    reviewer_pr_map: &HashMap<String, u64>,
    ...
    active_coworkers
        ...
        .filter_map(|cw| {
            ...
            // Get health status
            let health_color = health_snapshot
                .get(&cw.name)
                .map(|h| {
                    if !h.is_alive {
                        "red"
...
// ============================================================================
// GraphQL query and PR data fetching
// ============================================================================
...
fn fetch_kanban_all_prs(
    ...
    let open_prs = parse_open_prs(repository, reviewer_assignments, repo_label);
    let merged_prs = parse_merged_prs(repository, repo_label);

    (open_prs, merged_prs)
}

/// Parse open PRs from GraphQL response into kanban JSON values.
fn parse_open_prs(
...
/// Parse merged PRs from GraphQL response into kanban JSON values.
fn parse_merged_prs(
    repository: &serde_json::Value,
    repo_label: Option<&str>,
) -> Vec<serde_json::Value> {
...
// ============================================================================
// PR metadata extraction helpers
// ============================================================================
...
pub(super) fn build_pr_task_map(prs: &[serde_json::Value]) -> HashMap<u32, u64> {
    prs.iter()
        .filter_map(|pr| {
            let title = pr.get("title")?.as_str()?;
            let pr_number = pr.get("number")?.as_u64()?;
...
pub(super) fn extract_coworker_from_pr_body(body: &str) -> Option<String> {
    let marker = "midtown:";
...
pub(super) fn extract_reviewer_from_pr_comments(
...
pub(super) fn kanban_ci_status(checks: &[serde_json::Value]) -> &'static str {
    if checks.is_empty() {
        return "unknown";
    }
...
mod tests {
    ...
    fn test_reviewer_displays_source_task_id() {
        ...
        let task_id = Some(reviewer_internal_task_id);
        ...
        assert_eq!(display_task_id, Some(source_task_id));
        assert_eq!(pr_number_opt, Some(pr_number));
        assert_ne!(display_task_id, Some(reviewer_internal_task_id));
    }
}
