...
pub async fn evaluate_tick(
    ...
    match event {
        ...
        DaemonEvent::TaskDispatchTick => {
            ...
            // not here. Fork SessionRecords are marked non-running during
            // cleanup_coworker_state before the snapshot is collected, so a
            // snapshot-based approach would never see dead forks.
