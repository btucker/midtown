...
/// Returns (task_id, pr_number) for building task_to_pr maps.
fn add_coworker_with_open_pr(
    ps: &mut DaemonPersistentState,
    name: &str,
    session_id: &str,
    pr_number: u64,
    task_to_pr: &mut HashMap<String, u64>,
) {
    let task_id = format!("task-{}", pr_number);
    ps.sessions.insert(
    ...
    task_to_pr.insert(task_id, pr_number);
}

#[test]
fn nudges_coworkers_with_open_prs_when_pr_merges() {
    let mut ps = DaemonPersistentState::default();
    let mut task_to_pr = HashMap::new();
    ps.tick_merged_pr_numbers.insert(100);
    add_coworker_with_open_pr(&mut ps, "lexington", "session-1", 200, &mut task_to_pr);
    add_coworker_with_open_pr(&mut ps, "park", "session-2", 201, &mut task_to_pr);

    let effects = collect_merge_rebase_nudge_effects(&ps, &task_to_pr);
...
fn does_not_nudge_coworker_whose_pr_merged() {
    ...
    add_coworker_with_open_pr(&mut ps, "lexington", "session-1", 200, &mut task_to_pr);
    // Also give lexington a merged PR — update the task_to_pr to point to #100
    ps.sessions.get_mut("session-1").unwrap().pr_number = Some(100);
    // Update task_to_pr so lexington's task maps to PR #100 (the merged one)
    task_to_pr.insert("task-200".to_string(), 100);
...
fn does_not_nudge_coworkers_without_sessions() {
    ...
    let task_id = "task-200".to_string();
    ps.sessions.insert(
        "session-1".to_string(),
        SessionRecord {
...
fn only_nudges_for_new_merged_prs_not_processed_ones() {
    ...
    ps.tick_rebase_nudge_processed_prs.insert(100);
    add_coworker_with_open_pr(&mut ps, "lexington", "session-1", 300, &mut task_to_pr);
