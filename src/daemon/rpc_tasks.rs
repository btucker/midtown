//! RPC handlers for task operations (task.create, task.update, task.done, task.claim,
//! task.request, task.metadata).
//!
...
/// Handle coworker.report-state for Completed phase.
///
/// Called from the report-state handler when a coworker transitions to Completed.
/// Handles task completion logic: checks for open PRs, nudges if no PR exists,
/// and clears coworker assignments.
pub(super) async fn handle_completed_phase(name: &str, task_id: Option<u32>, state: &DaemonState) {
