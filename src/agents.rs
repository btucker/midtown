...
//!    Loaded from `.claude/agents/midtown-*.md` (project-level) or
//!    `~/.claude/agents/midtown-*.md` (user-level), with compiled-in fallback.
//!    For Claude/z.ai sessions, Layer 1 is provided via `--agent <name>` and
//!    the CLI loads the definition file directly. The public assembly functions
//!    still use the old-format files (`agents/*.md`) for the Codex path.
//!
