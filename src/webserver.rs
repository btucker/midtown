...
//! - `GET /api/projects/:name/channels/:channel_name/workflow` - Channel workflow info (script, mermaid, plugins)
...
///
/// These are compiled into the binary so the webserver can serve the default
...
///
/// Path: `/api/projects/:name/channels/:channel_name/workflow`
///
...
async fn project_channel_workflow(
    Path((name, channel_name)): Path<(String, String)>,
) -> Result<Json<serde_json::Value>, StatusCode> {
    if !is_valid_path_segment(&name) || !is_valid_path_segment(&channel_name) {
        return Err(StatusCode::BAD_REQUEST);
    ...
    let project_dir = crate::paths::projects_dir_for_repo(&name);

    // Resolve workflow script with 2-level local priority
    // (repo-committed paths need project_root which the webserver doesn't have)
    let (script_source, script_path, script_content) = {
        // 1. Channel-specific, local
        let channel_path = project_dir
            .join("channels")
            .join(&channel_name)
            .join("workflow.py");
        if channel_path.exists() {
            let content = std::fs::read_to_string(&channel_path).unwrap_or_default();
            (
                "channel-local",
                Some(channel_path.to_string_lossy().to_string()),
                content,
            )
        } else {
            // 2. Project default, local
            let project_path = project_dir.join("workflow.py");
            if project_path.exists() {
                ...
                // 3. Fall back to compiled-in default
                ("default", None, DEFAULT_WORKFLOW_CONTENT.to_string())
            }
        }
    };

    // For the mermaid diagram, use the embedded default for now.
    // Custom workflows would need Python invocation to generate their diagram.
    let mermaid = DEFAULT_WORKFLOW_MERMAID;

    // Discover plugins at project and channel level
    ...
    let plugin_dirs = [
        ...
        ("project-local", project_dir.join("plugins")),
    ];
...
pub async fn run(config: WebserverConfig) -> std::result::Result<(), Box<dyn std::error::Error>> {
    ...
    let api = Router::new()
        ...
        .route(
            "/projects/{name}/channels/{channel_name}/workflow",
            get(project_channel_workflow),
        )
...
mod tests {
    ...
    // --- project_channel_workflow handler tests ---

    #[tokio::test]
    async fn test_channel_workflow_rejects_invalid_names() {
        let result =
            project_channel_workflow(Path(("../etc".to_string(), "web".to_string()))).await;
        assert_eq!(result.unwrap_err(), StatusCode::BAD_REQUEST);

        let result =
            project_channel_workflow(Path(("proj".to_string(), "../etc".to_string()))).await;
        assert_eq!(result.unwrap_err(), StatusCode::BAD_REQUEST);
    }
    ...
    async fn test_channel_workflow_returns_default_when_no_custom() {
        ...
        // Create the projects dir so paths resolve
        let project_dir = tmp.path().join("projects").join("test-proj");
        std::fs::create_dir_all(&project_dir).unwrap();

        let result = project_channel_workflow(Path(("test-proj".to_string(), "web".to_string())))
            .await
            .unwrap();
    ...
    async fn test_channel_workflow_returns_channel_local_script() {
        ...
        assert_eq!(data["script_content"], "# custom channel workflow");
    }

    #[tokio::test]
    async fn test_channel_workflow_returns_project_local_script() {
        use crate::paths::set_test_midtown_base_dir;
        ...
        let project_dir = tmp.path().join("projects").join("test-proj");
        std::fs::create_dir_all(&project_dir).unwrap();
        std::fs::write(project_dir.join("workflow.py"), "# custom project workflow").unwrap();
        ...
        assert_eq!(data["script_content"], "# custom project workflow");
    }
    ...
    async fn test_channel_workflow_discovers_plugins() {
        ...
        let project_dir = tmp.path().join("projects").join("test-proj");
        let plugin_dir = project_dir.join("plugins");
