...
mod tests {
    ...
    fn test_calculate_input_bar_height_wraps_long_text() {
        ...
        // Available width = 80 - 2 (borders) - 2 (prompt "> ") - 1 (cursor "_") = 75
        let long_text = "a".repeat(150); // Should wrap to 2 lines
        let height = calculate_input_bar_height(&long_text, 80);
        assert_eq!(
            height, 4,
            "150 chars should wrap to 2 lines: 2 + 2 borders = 4"
        );
    }
