...
#[test]
fn spawn_accepts_call_in_alias() {
    use clap::Parser;
    let cli = TestAgentCli::try_parse_from(["test", "call-in"]).unwrap();
    assert!(matches!(cli.command, AgentCommand::Spawn { .. }));
}

#[test]
fn stop_accepts_break_alias() {
    use clap::Parser;
    let cli = TestAgentCli::try_parse_from(["test", "break", "park"]).unwrap();
    match cli.command {
        AgentCommand::Stop { name } => assert_eq!(name, "park"),
        other => panic!("Expected Stop, got {:?}", other),
    }
}

#[test]
fn show_accepts_view_alias() {
    use clap::Parser;
    let cli = TestAgentCli::try_parse_from(["test", "view", "park"]).unwrap();
    match cli.command {
        AgentCommand::Show { target, watch } => {
            assert_eq!(target, "park");
            assert!(!watch);
        }
...
fn ls_accepts_list_alias() {
    ...
    assert!(matches!(cli.command, AgentCommand::Ls));
}
