...
impl DaemonClient {
    ...
    pub fn channel_unarchive(&self, name: &str) -> Result<Response, String> {
        self.send(
            "channel.unarchive",
    ...
    /// and `log_path` (path to the JSONL log file, for `--watch` mode).
    pub fn session_view_raw(&self, target: &str) -> Result<Value, String> {
        self.send_raw(
            "session.view",
            Some(serde_json::json!({ "target": target })),
        )
    ...
    pub fn pr_review_post(&self, pr_number: u64, body: &str) -> Result<Response, String> {
        self.send(
            "pr.review-post",
            Some(serde_json::json!({ "pr": pr_number, "body": body })),
        )
    }
...
fn parse_daemon_response(value: Value) -> Result<Response, String> {
    ...
    if value.is_array()
        && let Ok(parsed) =
            serde_json::from_value::<Response>(serde_json::json!({ "messages": value.clone() }))
    {
        return Ok(parsed);
    }
