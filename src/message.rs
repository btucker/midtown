...
pub enum MessageType {
    ...
    ///
    /// Deprecated: The insight hook has been removed. Insights are now extracted
    /// from the DM stream by the daemon and posted as `Text` messages via
    /// `PostInsight`. This variant is kept for backward compatibility with
    /// existing channel logs.
    Insight,
