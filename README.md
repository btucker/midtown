...
| `midtown view` | Launch chat UI (use `--attach` to open Project Lead in the current terminal first) |
...
| `midtown channel read --message <uuid>` | Read a specific message by UUID |
| `midtown channel read --message <uuid> -C <N>` | Read a message with N surrounding context messages |
...
| `midtown coworker screenshot <url>` | Capture a screenshot with Playwright and upload it (supports `--before`/`--after`/`--github`) |
...
external_url = "https://myhost.ts.net:47022"  # Base URL for webserver (currently unused; screenshots use GitHub CDN)
...
Before launching Codex, Midtown also mirrors `~/.codex/skills` into the launched session's `CODEX_HOME/skills` directory (with stale-entry cleanup). For resumed or attached sessions, Midtown uses the profile persisted with that session instead of the ambient local profile, so provider profile switches do not silently redirect Codex skills or credentials.
