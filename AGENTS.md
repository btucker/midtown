...
- **Check code coverage before opening a PR.** Run `./scripts/coverage-diff.sh` to generate a branch-based coverage diff (requires `cargo-llvm-cov` and `diff-cover`). This shows coverage only for lines changed relative to `origin/main`. Review the HTML report at `target/diff-coverage/index.html`. New code should have reasonable coverage — if a function is untestable at the unit level (e.g., async functions requiring full `DaemonState`), note it in the PR description.
